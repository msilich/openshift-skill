...
OADP backs up and restores Kubernetes resources and internal images, at the granularity of a namespace, by using the version of Velero that is appropriate for the version of OADP you install, according to the table in [OADP-Velero-OpenShift Container Platform version relationship](application_backup_and_restore/troubleshooting/oadp-cli-tool.md#velero-oadp-version-relationship_oadp-cli-tool). OADP backs up and restores persistent volumes (PVs) by using snapshots or Restic. For details, see [OADP features](application_backup_and_restore/oadp-features-plugins.md#oadp-features_oadp-features-plugins).

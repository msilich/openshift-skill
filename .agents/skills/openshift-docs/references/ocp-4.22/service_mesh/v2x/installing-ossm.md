...
- Read the [Preparing to install Red Hat OpenShift Service Mesh](preparing-ossm-installation.md#preparing-ossm-installation) process.
...
- The Red Hat OpenShift Service Mesh Operator does not create the Service Mesh custom resource definitions (CRDs) until you deploy a Service Mesh control plane. You can use the `ServiceMeshControlPlane` resource to install and configure the Service Mesh components. For more information, see [Creating the ServiceMeshControlPlane](ossm-create-smcp.md#ossm-create-smcp).

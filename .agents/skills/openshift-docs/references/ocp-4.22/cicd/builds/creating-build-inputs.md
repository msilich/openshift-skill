...
- [Configuring project-scoped image pull secrets for mirrored registries](../../openshift_images/image-configuration.md#images-configuration-registry-mirror-project-secret_image-configuration)
...
> [!IMPORTANT]
> To prevent the contents of input secrets and config maps from appearing in build output container images, use build volumes in your [Docker build](build-strategies.md#builds-using-build-volumes_build-strategies-docker) and [source-to-image build](build-strategies.md#builds-using-build-volumes_build-strategies-s2i) strategies.
...
>
> To prevent the contents of input secrets and config maps from appearing in the build output container images and avoid this removal process altogether, [use build volumes](build-strategies.md#builds-using-build-volumes_build-strategies-docker) in your Docker build strategy instead.
...
- [Input secrets and config maps](creating-build-inputs.md#builds-input-secrets-configmaps_creating-build-inputs)

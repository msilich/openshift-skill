...
For more information about how to install the NMState Operator, see [Kubernetes NMState Operator](../networking_operators/k8s-nmstate-about-the-k8s-nmstate-operator.md#k8s-nmstate-about-the-k8s-nmstate-operator).
...
- [Example policy configurations for different interfaces](k8s-nmstate-updating-node-network-config.md#virt-nmstate-example-policy-configurations_k8s-nmstate-updating-node-network-config)

- [Removing an interface from nodes](k8s-nmstate-updating-node-network-config.md#virt-removing-interface-from-nodes_k8s-nmstate-updating-node-network-config)
...
- [Example for creating multiple interfaces in the same policy](k8s-nmstate-updating-node-network-config.md#virt-example-nmstate-multiple-interfaces_k8s-nmstate-updating-node-network-config)

- [Examples of different IP management methods in policies](k8s-nmstate-updating-node-network-config.md#virt-example-nmstate-IP-management_k8s-nmstate-updating-node-network-config)
...
- [Configuring an SR-IOV network device](../hardware_networks/configuring-sriov-device.md#configuring-sriov-device)

- [Configuring hardware offloading](../hardware_networks/configuring-hardware-offloading.md#configuring-hardware-offloading)
...
To simplify network administration and reduce configuration errors in OpenShift Container Platform, configure routes by using names instead of Virtual Routing and Forwarding (VRF) routing table ID numbers.
...
`spec.nodeSelector.vrf`
Specifies the application of the policy only to nodes that have the `vrf: true` label. This configuration prevents accidental network changes on nodes that do not require VRF support.

`spec.desiredState.interfaces.type`
Specifies the creation of a VRF-type interface.

`spec.desiredState.interfaces.vrf.route-table-id`
Specifies the `route-table-id` that the `vrf` name maps to in the routing table.

`spec.desiredState.routes.config.route-type`
Specifies the route type. NetworkManager supports the `unicast`, `local`, `blackhole`, `unreachable`, `prohibit`, and `throw` route types. The default is unicast. The example sets a `blackhole` route that drops any packets within the IP address range of this route. The range for the `blackhole` route is the destination CIDR, such as `198.51.200.0/24`.

`spec.desiredState.routes.config.vrf-name`
...
- [About virtual routing and forwarding](../multiple_networks/about-virtual-routing-and-forwarding.md#cnf-about-virtual-routing-and-forwarding_about-virtual-routing-and-forwarding)

- [Exposing a service through a network VRF](../ingress_load_balancing/metallb/metallb-configure-bgp-peers.md#nw-metallb-bgp-peer-vrf_configure-metallb-bgp-peers)

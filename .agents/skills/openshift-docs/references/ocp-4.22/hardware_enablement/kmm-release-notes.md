...
- In this release, wildcard and glob pattern support for the `filesToSign` field has been added for signing kernel modules in a specific folder. Previously, you had to specify the exact path to each `.ko` file you wanted to sign. Now, you can add the full path to explicit files, as previously required, or any glob patterns supported by the `Ash` shell.
  ...
  For more information, see [Specifying files to sign](kmm-kernel-module-management.md#kmm-specifying-files-to-sign_kernel-module-management-operator).
...
- In this release, KMM configurations are now persistent after cluster and KMM Operator upgrades and redeployments of KMM.
  ...
  For more information, see [Configuring the Kernel Module Management Operator](kmm-kernel-module-management.md#kmm-configuring-kmmo_kernel-module-management-operator).
...
- A requirement when creating a kmod image is that both the `.ko` kernel module files and the `cp` binary must be included, which is required for copying files during the image loading process. For more information, see [Creating a kmod image](kmm-kernel-module-management.md#kmm-creating-kmod-image_kernel-module-management-operator).

...
2.  Retrieve the user name and password of the bare-metal node’s baseboard management controller.

3.  Create `base64` strings from the user name and password:
...
- [Optional: Configuring host network interfaces in the install-config.yaml file](ipi/ipi-install-installation-workflow.md#configuring-host-network-interfaces-in-the-install-config-yaml-file_ipi-install-installation-workflow)

- [Automatically scaling machines to the number of available bare-metal hosts](../../scalability_and_performance/managing-bare-metal-hosts.md#automatically-scaling-machines-to-available-bare-metal-hosts_managing-bare-metal-hosts)
...
1.  Ensure that the Bare Metal Operator is available:
    ...
    baremetal   4.22     True          False      False   3d15h
    ```
...
- [Replacing an unhealthy etcd member](../../backup_and_restore/control_plane_backup_and_restore/replacing-unhealthy-etcd-member.md#replacing-the-unhealthy-etcd-member)

- [Backing up etcd](../../backup_and_restore/control_plane_backup_and_restore/backing-up-etcd.md#backing-up-etcd-data_backup-etcd)
...
- [BMC addressing](ipi/ipi-install-installation-workflow.md#bmc-addressing_ipi-install-installation-workflow)

...
<a id="secured-cluster-authentication-options_init-bundle-ocp"></a>
...
Before you set up a secured cluster, you must generate a CRS or an init bundle. The secured cluster then uses the contained secrets for initial authentication with Central. You can generate a CRS or an init bundle by using either the RHACS portal or the `roxctl` CLI.
...
<a id="generate-crs_init-bundle-ocp"></a>

# Cluster registration secrets overview

A cluster registration secret (CRS) is a credential used to authenticate secured clusters with Central. You can generate a CRS by using the RHACS portal or by using the `roxctl` CLI.
...
Retrieval  
You cannot retrieve a previously generated CRS from Central. Central stores metadata about the CRS, but it does not retain the secret data itself. If you lose the generated file, you must generate a new CRS.

Secure storage  
Because the generated file contains sensitive secrets and cannot be recovered if lost, you must store it in a secure location.

Reuse  
You can use a single CRS file to configure and authenticate more than one secured cluster.

> [!NOTE]
> You must have the `Admin` user role to generate a CRS.

<a id="portal-generate-crs_init-bundle-ocp"></a>
...
<a id="crs-generate-roxctl_init-bundle-ocp"></a>
...
<a id="crs-apply-secured-cluster_init-bundle-ocp"></a>
...
<a id="generate-init-bundle_init-bundle-ocp"></a>

# Cluster registration secret or init bundle generation methods

You can generate a cluster registration secret (CRS) or an init bundle to authenticate your secured clusters by using either the Red Hat Advanced Cluster Security for Kubernetes (RHACS) portal or the `roxctl` CLI. Both methods produce a YAML file that contains secrets. This file is required to establish the secure communication channel between Central and secured clusters and perform the initial secured cluster registration.

RHACS portal  
This web-based method is suitable for interactive generation. You can select your target platform and installation method through the **Platform Configuration** menu.

`roxctl` CLI  
This command-line method is suitable for scripting and automation. It requires the configuration of the `ROX_API_TOKEN` and `ROX_CENTRAL_ADDRESS` environment variables. You can specify the output format for Helm or Operator installations by using specific command flags.
...
<a id="portal-generate-init-bundle_init-bundle-ocp"></a>
...
<a id="roxctl-generate-init-bundle_init-bundle-ocp"></a>
...
<a id="create-resource-init-bundle_init-bundle-ocp"></a>
...
<a id="next-steps_init-bundle-ocp_init-bundle-ocp"></a>

# Next steps

Install RHACS secured cluster services in all clusters that you want to monitor.

<a id="additional-resources_init-bundle-ocp"></a>

# Additional resources

- [Installing secured cluster services for RHACS on Red Hat OpenShift](install-secured-cluster-ocp.md)

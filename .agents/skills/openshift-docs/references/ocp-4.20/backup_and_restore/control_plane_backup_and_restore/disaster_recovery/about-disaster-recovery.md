...
[Quorum restoration](quorum-restoration.md#dr-quorum-restoration)
...
> [!NOTE]
> If you have a majority of your control plane nodes still available and have an etcd quorum, then [replace a single unhealthy etcd member](../replacing-unhealthy-etcd-member.md#replacing-unhealthy-etcd-member).

[Restoring to a previous cluster state](scenario-2-restoring-cluster-state.md#dr-restoring-cluster-state)
...
If applicable, you might also need to [recover from expired control plane certificates](scenario-3-expired-certs.md#dr-recovering-expired-certs).
...
>
> Prior to performing a restore, see [About restoring cluster state](scenario-2-restoring-cluster-state.md#dr-scenario-2-restoring-cluster-state-about_dr-restoring-cluster-state) for more information on the impact to the cluster.

[Recovering from expired control plane certificates](scenario-3-expired-certs.md#dr-recovering-expired-certs)

...
If your cluster fails to recover, follow the steps to [restore to a previous cluster state](control_plane_backup_and_restore/disaster_recovery/scenario-2-restoring-cluster-state.md#dr-restoring-cluster-state).

# Prerequisites

- You have [gracefully shut down your cluster](graceful-cluster-shutdown.md#graceful-shutdown-cluster).
...
- See [Restoring to a previous cluster state](control_plane_backup_and_restore/disaster_recovery/scenario-2-restoring-cluster-state.md#dr-restoring-cluster-state) for how to use an etcd backup to restore if your cluster failed to recover after restarting.

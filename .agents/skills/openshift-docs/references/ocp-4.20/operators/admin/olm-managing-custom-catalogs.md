...
Cluster administrators and Operator catalog maintainers can create and manage custom catalogs packaged using the [bundle format](../understanding/olm-packaging-format.md#olm-bundle-format_olm-packaging-format) on Operator Lifecycle Manager (OLM) in OpenShift Container Platform.
...
- [Red Hat-provided Operator catalogs](../understanding/olm-rh-catalogs.md#olm-rh-catalogs)

</div>

# Prerequisites

- You have installed the [`opm` CLI](../../cli_reference/opm/cli-opm-install.md#cli-opm-install).
...
>
> Many of the `opm` subcommands and flags for working with the SQLite database format, such as `opm index prune`, do not work with the file-based catalog format. For more information about working with file-based catalogs, see [Operator Framework packaging format](../understanding/olm-packaging-format.md#olm-file-based-catalogs_olm-packaging-format) and [Mirroring images for a disconnected installation using the oc-mirror plugin](../../disconnected/installing-mirroring-disconnected.md#installing-mirroring-disconnected).
...
- [Packaging format → Schemas → olm.deprecations schema](../understanding/olm-packaging-format.md#olm-deprecations-schema_olm-packaging-format)

- [Mirroring images for a disconnected installation using the oc-mirror plugin → Keeping your mirror registry content updated](../../disconnected/installing-mirroring-disconnected.md#updating-mirror-registry-content)

- [Adding a catalog source to a cluster](../../disconnected/using-olm.md#olm-creating-catalog-from-index_olm-restricted-networks)
...
1.  Update the existing index by adding bundle images:
    ...
    - The `--pull-tool` flag specifies the tool used to pull container images.
      ...
      `<existing_tag>`
      Specifies a previously pushed image tag, such as `4.17`.
...
3.  Run the following command to prune the source index of all but the specified packages:
    ...
    ``` text
    $ opm index prune \
        -f registry.redhat.io/redhat/redhat-operator-index:v4.17 \
        -p advanced-cluster-management,jaeger-product,quay-operator \
        [-i registry.redhat.io/openshift4/ose-operator-registry-rhel9:v4.17] \
        -t <target_registry>:<port>/<namespace>/redhat-operator-index:v4.17
    ```
...
In OpenShift Container Platform 4.17, namespaces do not have restricted pod security enforcement by default and the default catalog source security mode is set to `legacy`.
...
- See [What is a secret?](../../cicd/builds/creating-build-inputs.md#builds-secrets-overview_creating-build-inputs) for more information on the types of secrets, including those used for registry credentials.

- See [Updating the global cluster pull secret](../../openshift_images/managing_images/using-image-pull-secrets.md#images-update-global-pull-secret_using-image-pull-secrets) for more details on the impact of changing this secret.

- See [Allowing pods to reference images from other secured registries](../../openshift_images/managing_images/using-image-pull-secrets.md#images-allow-pods-to-reference-images-from-secure-registries_using-image-pull-secrets) for more details on linking pull secrets to service accounts per namespace.

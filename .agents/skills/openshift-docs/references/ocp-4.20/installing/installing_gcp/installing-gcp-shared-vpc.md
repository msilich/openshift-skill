...
In OpenShift Container Platform version 4.20, you can install a cluster into a shared Virtual Private Cloud (VPC) on Google Cloud. In this installation method, the cluster is configured to use a VPC from a different Google Cloud project. A shared VPC enables an organization to connect resources from multiple projects to a common VPC network. You can communicate within the organization securely and efficiently by using internal IP addresses from that network.
...
- [Required Google Cloud permissions for shared VPC installations](installing-gcp-account.md#minimum-required-permissions-ipi-gcp-xpn_installing-gcp-account)

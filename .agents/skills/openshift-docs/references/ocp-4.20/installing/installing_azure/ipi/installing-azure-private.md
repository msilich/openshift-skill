...
In OpenShift Container Platform version 4.20, you can install a private cluster into an existing Azure Virtual Network (VNet) on Microsoft Azure. The installation program provisions the rest of the required infrastructure, which you can further customize. To customize the installation, you modify parameters in the `install-config.yaml` file before you install the cluster.
...
In OpenShift Container Platform 4.20, you can deploy a cluster into an existing Microsoft Azure Virtual Network (VNet). Deployments in an existing VNet require existing subnets and routing rules.
...
- For more details about Accelerated Networking, see [Accelerated Networking for Microsoft Azure VMs](../../../machine_management/creating_machinesets/creating-machineset-azure.md#machineset-azure-accelerated-networking_creating-machineset-azure).
...
- To manage long-term cloud credentials manually, follow the procedure in [Manually creating long-term credentials](installing-azure-private.md#manually-create-iam_installing-azure-private).

- To implement short-term credentials that are managed outside the cluster for individual components, follow the procedures in [Configuring an Azure cluster to use short-term credentials](installing-azure-private.md#installing-azure-with-short-term-creds_installing-azure-private).
...
- For the list of permissions needed to create a private storage endpoint, see [Required Azure permissions for installer-provisioned infrastructure](../installing-azure-account.md#minimum-required-permissions-ipi-azure_installing-azure-account).

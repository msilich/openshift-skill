...
> [!WARNING]
> In OpenShift Container Platform 4.20, the installation program uses Cluster API instead of Terraform to provision cluster infrastructure during installations on AWS. Installing a cluster on AWS into a secret or top-secret region by using the Cluster API implementation has not been tested as of the release of OpenShift Container Platform 4.20. This document will be updated when installation into a secret region has been tested.
>
...
3.  Export the version of RHCOS you uploaded to Amazon S3 as an environment variable by running the following command:
    ...
    Replace `<version>` with the RHCOS VMDK version, such as `4.20.0`.

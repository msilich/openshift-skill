...
In FIO 1.3.1, increasing nodes in IBM Z® clusters might result in `Failed` File Integrity node status. For more information, see [Adding nodes in IBM Power® clusters can result in failed File Integrity node status](https://access.redhat.com/solutions/7028861).
...
- The File Integrity Operator Custom Resource (CR) now contains an `initialDelay` feature that specifies the number of seconds to wait before starting the first AIDE integrity check. For more information, see [Creating the FileIntegrity custom resource](file-integrity-operator-understanding.md#understanding-file-integrity-custom-resource_file-integrity-operator).

- The File Integrity Operator is now stable and the release channel is upgraded to `stable`. Future releases will follow [Semantic Versioning](https://semver.org/). To access the latest release, see [Updating the File Integrity Operator](file-integrity-operator-updating.md#olm-preparing-upgrade_file-integrity-operator-updating).
...
- [Updating the File Integrity Operator](file-integrity-operator-updating.md#olm-preparing-upgrade_file-integrity-operator-updating)

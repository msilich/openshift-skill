...
1.  Create a standalone `PolicyGenTemplate` custom resource (CR) that contains the source file `validatorCRs/informDuValidator.yaml`. You only need one standalone `PolicyGenTemplate` CR for each cluster type. For example, this CR applies a validator inform policy for single-node OpenShift clusters:
    ...
    `spec.bindingRules`
    Specifies the binding rules. The `group-du-*` label defined in `bindingRules` must exist in the `SiteConfig` files.
...
- [Configuring node power consumption and realtime processing with workload hints](../../scalability_and_performance/cnf-tuning-low-latency-nodes-with-perf-profile.md#configuring-workload-hints_cnf-tuning-low-latency-nodes-with-perf-profile)
...
- [Configuring host firmware for low latency and high performance](../ztp-reference-cluster-configuration-for-vdu.md#ztp-du-configuring-host-firmware-requirements_sno-configure-for-vdu)
...
- [Configuring power saving for nodes that run colocated high and low priority workloads](../../scalability_and_performance/cnf-tuning-low-latency-nodes-with-perf-profile.md#cnf-configuring-power-saving-for-nodes_cnf-tuning-low-latency-nodes-with-perf-profile)
...
- [Using PolicyGenTemplate CRs to override source CRs content](ztp-advanced-policy-config.md#ztp-using-pgt-to-update-source-crs_ztp-advanced-policy-config)
...
- [OpenShift image registry overview](../../registry/index.md#registry-overview)
...
- [OpenShift Container Platform registry overview](../../registry/index.md#registry-overview)
...
- [Accessing the registry](../../registry/accessing-the-registry.md#accessing-the-registry)

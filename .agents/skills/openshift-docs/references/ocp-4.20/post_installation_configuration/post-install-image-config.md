...
2.  Fetch the tags for every imagestream in the `openshift` namespace by running the following command:
    ...
    ``` terminal
    $ oc get is <image-stream-name> -o jsonpath="{range .spec.tags[*]}{'\t'}{.from.name}{'\n'}{end}" -n openshift
    ```
...
4.  Verify the scheduling status of the periodic import by running the following command:
    ...
    ``` terminal
    oc get imagestream <image-stream-name> -o jsonpath="{range .spec.tags[*]}Tag: {'\t'}Scheduled: {.importPolicy.scheduled}{'\n'}{end}" -n openshift
    ```

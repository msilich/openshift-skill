...
Optionally, you can control swap memory usage by OpenShift Container Platform workloads on those nodes by setting the `swapBehavior` parameter to one of the following values:

- `NoSwap` prevents OpenShift Container Platform worloads from using swap memory.

- `LimitedSwap` allows OpenShift Container Platform workloads that fall under the Burstable QoS class to use swap memory.

Regardless of the `swapBehavior` setting, any workloads that are not managed by OpenShift Container Platform on that node can still use swap memory if the `failSwapOn` parameter is `false`.

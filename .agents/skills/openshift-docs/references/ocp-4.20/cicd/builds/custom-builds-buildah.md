...
With OpenShift Container Platform 4.17, a docker socket will not be present on the host nodes. This means the *mount docker socket* option of a custom build is not guaranteed to provide an accessible docker socket for use within a custom build image.
...
- Review how to [grant custom build permissions](securing-builds-by-strategy.md#securing-builds-by-strategy).

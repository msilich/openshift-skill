...
> [!IMPORTANT]
> Before you run a latency checkup, you must first [create a bridge interface](../vm_networking/virt-connecting-vm-to-linux-bridge.md#virt-connecting-vm-to-linux-bridge) on the cluster nodes to connect the VM’s secondary interface to any interface on the node. If you do not create a bridge interface, the VMs do not start and the job fails.
...
- [Collecting data for Red Hat Support](../support/virt-collecting-virt-data.md#virt-collecting-virt-data)

- [Using the `must-gather` tool for OpenShift Virtualization](../support/virt-collecting-virt-data.md#virt-using-virt-must-gather_virt-collecting-virt-data)

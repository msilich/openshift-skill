...
3.  Delete the associated `KubeletConfig` custom resource (CR) by running the following command:

    ``` terminal
    $ oc delete kubeletconfig custom-config
    ```

4.  Wait for the worker nodes to reconcile, by running the following command and observing the output:

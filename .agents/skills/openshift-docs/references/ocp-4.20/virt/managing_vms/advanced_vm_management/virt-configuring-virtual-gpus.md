...
This method exclusively uses the NVIDIA GPU Operator to configure mediated devices. To use this method, refer to [NVIDIA GPU Operator with OpenShift Virtualization](https://docs.nvidia.com/datacenter/cloud-native/openshift/latest/openshift-virtualization.html) in the NVIDIA documentation.
...
When using the OpenShift Virtualization method, you still configure the GPU Operator by following [the NVIDIA documentation](https://docs.nvidia.com/datacenter/cloud-native/openshift/latest/openshift-virtualization.html). However, this method differs from the NVIDIA documentation in the following ways:

- You must not overwrite the default `disableMDEVConfiguration: false` setting in the `HyperConverged` custom resource (CR).

  > [!IMPORTANT]
  > Setting this feature gate as described in the [NVIDIA documentation](https://docs.nvidia.com/datacenter/cloud-native/openshift/latest/openshift-virtualization.html#prerequisites) prevents OpenShift Virtualization from configuring mediated devices.
...
- [Configuring PCI passthrough](virt-configuring-pci-passthrough.md#virt-configuring-pci-passthrough)

...
4.  Verify that the `MachineOSConfig` object contains a reference to the new custom layered image by running the following command:
...
5.  Verify that the appropriate nodes are using the new custom layered image:
...
- [About checking machine config node status](index.md#checking-mco-node-status_machine-config-overview)

...
You can migrate from [OpenShift Container Platform 3 to 4.20](../../migrating_from_ocp_3_to_4/about-migrating-from-3-to-4.md#about-migrating-from-3-to-4) and between OpenShift Container Platform 4 clusters.
...
- Microsoft Azure storage is unavailable if you create more than 400 migration plans. The `MigStorage` custom resource displays the following message: `The request is being throttled as the limit has been reached for operation type`. ([**BZ#1977226**](https://bugzilla.redhat.com/show_bug.cgi?id=1977226))

- If a migration fails, the migration plan does not retain custom persistent volume (PV) settings for quiesced pods. You must manually roll back the migration, delete the migration plan, and create a new migration plan with your PV settings. ([**BZ#1784899**](https://bugzilla.redhat.com/show_bug.cgi?id=1784899))

- PV resizing does not work as expected for AWS gp2 storage unless the `pv_resizing_threshold` is 42% or greater. ([**BZ#1973148**](https://bugzilla.redhat.com/show_bug.cgi?id=1973148))

---
name: openshift-docs
description: Search and use the bundled official OpenShift Container Platform 4.22 documentation without network access. Use for OCP 4.22 installation, configuration, administration, security, networking, storage, Operators, upgrades, troubleshooting, CLI, disconnected-environment, and product-behavior questions that should be grounded in version-specific Red Hat documentation.
---

# OpenShift Container Platform 4.22 documentation

Use the bundled snapshot as the primary authority for OCP 4.22 product behavior and procedures.

1. Resolve all bundled paths relative to this skill directory, never relative to the process working directory.
2. Start with [the compressed documentation map](references/ocp-4.22/AGENTS.md). Read only the topic files relevant to the question.
...
5. Consult [SOURCE.json](references/ocp-4.22/SOURCE.json) when provenance, freshness, coverage, or licensing matters.

Keep these boundaries explicit:

- This snapshot covers OCP 4.22 only. State that limitation for another requested version.

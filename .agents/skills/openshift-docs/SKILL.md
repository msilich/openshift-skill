...
2. Start with [the OCP 4.22 map](references/ocp-4.22/AGENTS.md), [the GitOps 1.21 map](references/gitops-1.21/AGENTS.md) or [the Dev Spaces 3.29 map](references/devspaces-3.29/AGENTS.md). GitOps and Dev Spaces have their own release cadences; do not infer their versions from OCP. Read only relevant topic files.
...
5. Consult [OCP provenance](references/ocp-4.22/SOURCE.json), [GitOps provenance](references/gitops-1.21/SOURCE.json) or [Dev Spaces provenance](references/devspaces-3.29/SOURCE.json) when freshness, coverage or licensing matters. Missing local material is an explicit gap, not permission for a runtime download.
...
- The snapshots cover OCP 4.22, GitOps 1.21, Dev Spaces 3.29 and RHACS 4.11. State the limitation for another requested version. Bundling them together does not certify product compatibility.
